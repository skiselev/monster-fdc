...
* Install the card in your computer and connect floppy drives
* Power on the computer and launch the built-in Multi-Floppy BIOS configuration utility to configure controllers and floppy drives
* If needed, modify the OS configuration

## Configuration Jumpers

Monster FDC controller card is configured using jumpers. There are four blocks of jumpers that configure controller resources and two optional blocks that are used to connect +5V power supply to pin 3 of floppy disk connectors. Certain IBM PS/2 floppy drives are powered using this pin.
...
**Warning:** Do not connect JP6-JP7 jumpers unless you are certain that your floppy drive is powered using pin 3. Such drives normally do not have separate power connector.
...
The Multi-Floppy BIOS Extension ROM features a built-in configuration utility, that is used to configure floppy drive types connected to the controller card, and to set up the secondary FDC resources (IRQ, DMA channel). Press **F2** button during BIOS POST to access the configuration utility. The configuration utility has a simple command line interface. The configuration commands are executed by pressing the corresponding command keys (just the key, no need to type **Enter**). While at the command prompt, use **h** command to list available commands. Commands that require parameters, display appropriate prompts. When prompted, enter the required parameter, or press **ESC** key to exit to the main menu.
...
* Then, it will prompt for the logical drive number.
  * The logical drive number corresponds to the drive number from software (BIOS calls) perspective
  ...
  * The range of logical drive numbers depends on the number of already configured drives
    * Entering a number at the end of the range will add a drive to the end of the list
...
Normally, at the end of Power-on Self Test (POST) the System BIOS calls INT 19h to boot the OS. The System BIOS has a default handler for INT 19h.
...
Use **i** command to configure the IPL method.

#### Other Commands

* Use **p** command to print the current configuration
* Use **w** command to write the configuration to the flash ROM / EEPROM and reboot the system. Note that flash ROM / EEPROM needs to be writable - the JP5.7 jumper must be set, and on newer systems, the caching for the memory range where Multi-Floppy BIOS Extension ROM resides should be disabled
...
* Use **ESC** button while running a command to exit to the main menu
